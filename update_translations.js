...
// Read i18n.js
let i18n_content = fs.readFileSync('js/i18n.js', 'utf8');
...
for (const [key, vi_text] of Object.entries(new_translations)) {
    if (!i18n_content.includes(`'${key}':`) && !i18n_content.includes(`"${key}":`)) {
        const insert_str = `\n        '${key}': '${vi_text}',`;
        i18n_content = i18n_content.replace("'vi': {", "'vi': {" + insert_str);
    }
}

fs.writeFileSync('js/i18n.js', i18n_content);
